...
struct StoryBrain {
    ...
    let story =  [
        Story(title:"Your car has blown a tire on a winding road in the middle of nowhere with no cell phone reception. You decide to hitchhike. A rust pickup truck rumbles to a stop next to you. A man with a wide-brimmed hat and soulless eyes opens the passenger door for you and says: Need a ride, boy?.",
    ...
    mutating func nextStory(_ choice: Int){
        if(choice == 1){
            storyTrack = story[storyTrack].choice1Destination;
        }else {
            storyTrack = story[storyTrack].choice2Destination;
        }
    }
}
